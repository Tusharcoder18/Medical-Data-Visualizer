...
df['overweight'] = (df['weight'] / ((df['height']/100) ** 2)).apply(lambda x: int(x > 25))
...
def draw_cat_plot():
    ...
    reformat = {
    f'cardio_{cardio}' : data.groupby(['cardio', 'variable', 'value']).size().reset_index(name='total') for cardio, data in df_cat.groupby('cardio')
}
    df_cat = reformat['cardio_0'].append(reformat['cardio_1'])

    # Draw the catplot with 'sns.catplot()'
    g = sns.catplot(data=df_cat, kind='bar', x='variable', y='total', col='cardio', hue='value')
    fig, ax = g.fig, g.axes
...
def draw_heat_map():
    ...
    ax = sns.heatmap(corr, mask=mask, annot=True, fmt='0.1f',vmin=0.08, vmax=0.24, linewidths=0.5)
